...
open class TLPhotosPickerViewController: UIViewController {
    ...
    public var cloudRequestIDs = SynchronizedDictionary<IndexPath,PHImageRequestID>()
    ...
    public var queueForGroupedBy = DispatchQueue(label: "tilltue.photos.pikcker.queue.for.groupedBy", qos: .utility)
    ...
    public init() {
        super.init(nibName: "TLPhotosPickerViewController", bundle: TLBundle.resourceBundle(classCoder: TLPhotosPickerViewController.classForCoder()))
    }
...
extension TLPhotosPickerViewController {
    ...
    @objc open func makeUI() {
        registerNib(nibName: "TLPhotoCollectionViewCell", bundle: TLBundle.resourceBundle(classCoder: TLPhotoCollectionViewCell.classForCoder()))
    ...
    open func cancelAllCloudRequest() {
        self.cloudRequestIDs.forEach { (_, requestID) in
            self.photoLibrary.cancelPHImageRequest(requestID: requestID)
        }
